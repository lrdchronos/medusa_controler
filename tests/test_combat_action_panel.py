...
from src.ui.utils.ui_layout import PixelIconDrawer
...
class TestCombatActionPanel(unittest.TestCase):
    ...
    validando margens verticais da barra de HP e renderização escalonada dos ícones de condição.
    """
    ...
    def test_hp_bar_vertical_margins_and_layout(self, mock_outline, mock_filled, mock_draw_icon):
        """Verifica se a barra de vida possui margin_top = 10.0 e margin_bottom = 12.0 em relação a textos e botões."""
        panel_w = 600.0
        disp_top = 500.0
        ...
        # 2. Barra de HP: bar_y = (disp_top - 28.0) - margin_top(10.0) - bar_h/2(4.0) = disp_top - 42.0
        expected_bar_y = disp_top - 42.0
        expected_bar_w = panel_w - 60.0
        expected_bar_h = 8.0
        ...
        # 3. Botões de Dano: btn_dmg_y = (bar_y - bar_h/2) - margin_bottom(12.0) - dmg_btn_h/2(10.0) = disp_top - 68.0
        expected_btn_dmg_y = disp_top - 68.0
        found_dmg_btn = False
        for call_args in mock_filled.call_args_list:
            rect = call_args[0][0]
            cy = getattr(rect, "center_y", getattr(rect, "y", None))
            h = getattr(rect, "height", getattr(rect, "h", None))
            if cy is not None and h is not None:
                if abs(cy - expected_btn_dmg_y) < 0.1 and abs(h - 20.0) < 0.1:
                    found_dmg_btn = True
                    break
        self.assertTrue(found_dmg_btn, f"Botões de dano não encontrados no centro Y esperado {expected_btn_dmg_y}")
    ...
    def test_condition_icons_rendered_with_pixel_icon_drawer(self, mock_outline, mock_filled, mock_draw_icon):
        ...
        panel_w = 600.0
        disp_top = 500.0
        expected_cond_btn_y = disp_top - 148.0
