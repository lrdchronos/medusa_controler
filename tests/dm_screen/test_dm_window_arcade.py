...
class TestDMWindowArcade(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.session = SessionManager()
        cls.dm_window = DMWindow(session_manager=cls.session, width=1280, height=768)
        cls.player_window = PlayerWindow(session_manager=cls.session, dm_window=cls.dm_window, width=1024, height=768)
